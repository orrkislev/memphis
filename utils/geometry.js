...
function setColors(path, fillColor, strokeColor, includeChildren = true) {
    if (path.children && includeChildren) {
        path.children.forEach(p => setColors(p, fillColor, strokeColor))
    }
    path.fillColor = Array.isArray(fillColor) ? choose(fillColor) : fillColor
    path.strokeColor = Array.isArray(strokeColor) ? choose(strokeColor) : strokeColor
}

function uniteReplace(path1, path2, removeOther = true) {
    const temp = path1.unite(path2)
    path1.remove()
    if (removeOther) path2.remove()
    return temp
}

function subtractReplace(path1, path2, removeOther = true) {
    const temp = path1.subtract(path2)
    path1.remove()
    if (removeOther) path2.remove()
    return temp
}

function intersectReplace(path1, path2, removeOther = true) {
    const temp = path1.intersect(path2)
    path1.remove()
    if (removeOther) path2.remove()
    return temp
}